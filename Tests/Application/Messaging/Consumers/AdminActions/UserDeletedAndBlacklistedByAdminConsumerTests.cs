...
public class UserDeletedAndBlacklistedByAdminConsumerTests
{
    ...
    public async Task Consume_SendsDeleteAllUserDataCommand_ForTheEventsUserId()
    {
        ...
        var context = Substitute.For<ConsumeContext<UserDeletedAndBlacklistedByAdminEvent>>();
        context.Message.Returns(new UserDeletedAndBlacklistedByAdminEvent
        {
            UserId = userId, Email = "a@b.no", Name = "Ola", Reason = "Spam", DeletedAt = DateTime.UtcNow
        });
        var consumer = new UserDeletedAndBlacklistedByAdminConsumer(mediator);
