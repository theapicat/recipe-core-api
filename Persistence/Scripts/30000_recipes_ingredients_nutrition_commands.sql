...
CREATE OR REPLACE FUNCTION insert_nutrient_definition(
    p_id text,
    p_name text,
    p_unit text,
    p_decimal_precision int,
    p_parent_id text,
    p_source_url text
)
RETURNS void
LANGUAGE sql
AS $$
    INSERT INTO nutrient_definition (id, name, unit, decimal_precision, parent_id, source_url)
    VALUES (p_id, p_name, p_unit, p_decimal_precision, p_parent_id, p_source_url);
$$;

CREATE OR REPLACE FUNCTION update_nutrient_definition(
    p_id text,
    p_name text,
    p_unit text,
    p_decimal_precision int,
...
AS $$
    ...
    SET name = p_name,
        unit = p_unit,
        decimal_precision = p_decimal_precision,
        parent_id = p_parent_id,
        source_url = p_source_url
    WHERE id = p_id;
$$;

CREATE OR REPLACE FUNCTION delete_nutrient_definition(p_id text)
RETURNS integer
LANGUAGE sql
AS $$
    WITH d AS (DELETE FROM nutrient_definition WHERE id = p_id RETURNING 1) SELECT count(*)::integer FROM d;
$$;
